...
namespace CubeHack.Client
{
    static class TextureAtlas
    {
        ...
        {
            foreach (var layer in texture.Layers)
            {
                if (layer is SolidColorLayer)
                {
                    DrawLayer(graphics, (SolidColorLayer)layer, x, y);
                }
                else
                {
        ...
        static void DrawLayer(Graphics graphics, SolidColorLayer layer, int x, int y)
        {
            graphics.FillRectangle(BrushFromColor(layer.Color), x, y, x + _textureSize, y + _textureSize);
        }

        static Brush BrushFromColor(CubeHack.GameData.Color c)
        {
